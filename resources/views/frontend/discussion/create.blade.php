...
@section('section')
            ...
            <div class="tt-wrapper-inner">
                <h1 class="tt-title-border">
                    {{isset($question)?'Update Question':'Create New Question'}}
                </h1>
                <form action="{{isset($question)?route('question.update'):route('question.store')}}" class="form-default form-create-topic" method="post">
                    @csrf
                    @if(isset($question))
                        @method('PATCH')
                        @endif
                    <div class="form-group">
                        ...
                        <div class="tt-value-wrapper">
                            <input type="text" value="{{isset($question)?$question->title:old('title')}}" name="title" class="form-control  @error('title') is-invalid @enderror" id="inputTopicTitle" placeholder="Subject of your topic">
                            @error('title')
                    ...
                    <div class="pt-editor">
                        ...
                        <div class="form-group">
                            <input id="contents" type="hidden" class="form-control  @error('body') is-invalid @enderror" name="body" value="{{isset($question)?$question->body:old('body')}}">
                        ...
                        <div class="row">
                            <div class="col-auto ml-md-auto">
                                <button type="submit" class="btn btn-secondary btn-width-lg">
                                    {{isset($question)?'Update Question':'Create Question'}}</button>
                            </div>
                        </div>
                    </div>
                </form>
